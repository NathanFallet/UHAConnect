...
fun Route.apiUsers() {
    route("/users") {
        ...
        put("/{id}") {
            ...
            if (id != me.id && !me.role.hasPermission(Permission.USER_UPDATE)) {
            ...
            Database.dbQuery {
                Users
                    .update({ Users.id eq user.id }) {
                        ...
                        if (me.role.hasPermission(Permission.USER_UPDATE)) {
                            it[role] =
            ...
            if (newUser.role != user.role && newUser.role == RoleStatus.BANNED) {
                NotificationsPlugin.sendNotificationToUser(
                    newUser.id,
                    NotificationData(
                        title_loc_key = "notifications_ban_title",
                        body_loc_key = "notifications_ban_body",
                        body_loc_args = listOf(me.username)
                    )
                )
            }
